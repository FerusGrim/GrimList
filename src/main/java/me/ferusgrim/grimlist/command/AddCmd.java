...
public class AddCmd implements CommandExecutor {
    ...
    public CommandResult execute(final CommandSource src, CommandContext args) throws CommandException {
        ...
        src.sendMessage(Texts.of("Attempting to add " + playerName + " to the whitelist!"));
        ...
        this.plugin.getGame().getAsyncScheduler().runTask(this.plugin, new Runnable() {
            ...
            public void run() {
                ...
                if (plugin.getDatabase().isWhitelisted(playerId)) {
                    this.sendMessage("Player is already on the whitelist!");
                } else {
                    plugin.getDatabase().add(playerId, mod);
                    this.sendMessage(playerName + " has been added to the whitelist!");
                }
            }
    ...
    private String getModClassification(CommandSource src) {
